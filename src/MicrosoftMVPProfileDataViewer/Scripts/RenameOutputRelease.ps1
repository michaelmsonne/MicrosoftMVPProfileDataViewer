...
if (Test-Path -Path $FolderName -PathType Container) {   
    ...
    Get-ChildItem -Path "$OutputFolderPath\*Microsoft MVP Profile Data Viewer*Build at*.exe" -Recurse | Move-Item -Destination $FolderName
    Write-Host "Moved files $OutputFolderPath\*Microsoft MVP Profile Data Viewer*Build at*.exe to $FolderName"
} else {
...
Get-ChildItem -Path "$OutputFolderPath\*Microsoft MVP Profile Data Viewer*Build at*" -File | Where-Object CreationTime -lt (Get-Date).AddSeconds(-2) | Remove-Item -Force
